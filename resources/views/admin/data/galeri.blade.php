        ...
        <div class="table-container">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td><img src="{{ asset('storage/' . $item->image) }}" class="img-preview-table shadow-sm" alt="Foto"></td>
                        <td class="fw-bold">{{ $item->title }}</td>
                        <td>{{ $item->created_at->format('d M Y') }}</td>
                        <td>
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm btn-outline-warning" onclick="bukaModal('editModal{{$item->id}}')">Edit</button>
                                <form action="{{ route('admin.data.galeri.destroy', $item->id) }}" method="POST">
                                    @csrf @method('DELETE')
...
<div class="custom-pop-up poppins-wrapper" id="tambahModal">
    <div class="modal-dialog">
        <form action="{{ route('admin.data.galeri.store') }}" method="POST" class="modal-content" enctype="multipart/form-data">
            @csrf
...
<div class="custom-pop-up poppins-wrapper" id="editModal{{$item->id}}">
    <div class="modal-dialog">
        <form action="{{ route('admin.data.galeri.update', $item->id) }}" method="POST" class="modal-content" enctype="multipart/form-data">
            @csrf @method('PUT')
            <h6 class="fw-bold mb-3">Edit Galeri</h6>
            <input type="text" name="title" value="{{ $item->title }}" class="form-control mb-2" required>
            <img id="prevEdit{{$item->id}}" class="preview-img active-preview" src="{{ asset('storage/' . $item->image) }}">
            <input type="file" name="image" class="form-control mb-2" accept="image/*" onchange="previewFile(this, 'prevEdit{{$item->id}}')">
            <div class="d-flex justify-content-end mt-2">
                <button type="button" class="btn btn-secondary me-2" onclick="tutupModal('editModal{{$item->id}}')">Batal</button>
