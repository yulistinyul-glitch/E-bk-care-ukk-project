...
<div class="w-full max-w-md md:max-w-4xl flex flex-col md:flex-row md:gap-16 items-center p-4 md:p-8 md:border-2 md:border-white/40 md:rounded-[40px] md:backdrop-blur-sm ">
    ...
    <div class="bg-white w-full max-w-95 shadow-2xl rounded-4xl p-8 md:p-10">
        ...
        <?php if($errors->any()): ?>
            <input type="hidden" id="loginError" value="<?php echo e($errors->first()); ?>">
        <?php endif; ?>

        <div id="form-siswa" class="space-y-4 transition-all duration-300">
            <?php if($errors->any()): ?>
                <div id="errorAlert" class="bg-red-100 text-red-600 p-3 rounded-xl text-xs mb-2"><?php echo e($errors->first()); ?></div>
            <?php endif; ?>
            <?php if(session('success')): ?>
                <div class="bg-green-100 text-green-600 p-3 rounded-xl text-xs mb-2"><?php echo e(session('success')); ?></div>
            <?php endif; ?>

            <form action="<?php echo e(route('siswa.login.submit')); ?>" method="POST" autocomplete="off" novalidate>
                <?php echo csrf_field(); ?>
        ...
        <div id="form-guru" class="hidden space-y-4 transition-all duration-300">
            <form action="<?php echo e(route('gurubk.login.submit')); ?>" method="POST" autocomplete="off" novalidate>
                <?php echo csrf_field(); ?>
