            ...
            <form action="{{ route('admin.tentang.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <fieldset id="formFields" disabled>
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" class="form-control mb-2" value="{{ $data->title }}">
                        <input type="text" name="tagline" class="form-control mb-2" value="{{ $data->tagline }}">
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                <textarea name="desc_1" class="form-control" rows="4" placeholder="Deskripsi 1">{{ $data->desc_1 }}</textarea>
                            </div>
                            <div class="col-md-6 mb-2">
                                <textarea name="desc_2" class="form-control" rows="4" placeholder="Deskripsi 2">{{ $data->desc_2 }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 p-3 bg-light rounded align-items-start">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="visi_judul" class="form-control mb-2" value="{{ $data->visi_judul }}">
                            <input type="text" name="visi_tagline" class="form-control mb-2" value="{{ $data->visi_tagline }}">
                            <textarea name="visi_desc" class="form-control" rows="3">{{ $data->visi_desc }}</textarea>
                        </div>
                        <div class="col-md-4 text-center">
                            <label class="d-block small text-muted mb-1">Preview Foto Visi</label>
                            <img id="prevVisi" src="{{ $data->foto_visi ? asset('storage/'.$data->foto_visi) : '#' }}" class="img-preview" style="display:{{ $data->foto_visi ? 'block':'none' }}">
                    ...
                    <div class="row mb-4 p-3 bg-light rounded align-items-start">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="misi_judul" class="form-control mb-2" value="{{ $data->misi_judul }}">
                            <input type="text" name="misi_tagline" class="form-control mb-2" value="{{ $data->misi_tagline }}">
                            <textarea name="misi_desc" class="form-control" rows="3">{{ $data->misi_desc }}</textarea>
                        </div>
                        <div class="col-md-4 text-center">
                            <label class="d-block small text-muted mb-1">Preview Foto Misi</label>
                            <img id="prevMisi" src="{{ $data->foto_misi ? asset('storage/'.$data->foto_misi) : '#' }}" class="img-preview" style="display:{{ $data->foto_misi ? 'block':'none' }}">
