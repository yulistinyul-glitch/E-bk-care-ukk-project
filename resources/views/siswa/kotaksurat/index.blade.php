  ...
  {{-- List pesan --}}
    <div class="space-y-4">
        @forelse($surat as $item)
            <div onclick="showDetail('{{ $item->subject }}', '{{ $item->message }}')" 
                 class="bg-white border-2 {{ $item->read_at ? 'border-gray-100' : 'border-blue-400' }} p-5 rounded-3xl shadow-sm hover:shadow-md transition-all cursor-pointer flex items-center gap-4 group">
                
                <div class="p-3 {{ $item->read_at ? 'bg-gray-100 text-gray-400' : 'bg-blue-100 text-blue-600' }} rounded-2xl group-hover:scale-110 transition-transform">
                ...
                <div class="flex-1">
                    <h4 class="font-bold text-[#1A374C] {{ $item->read_at ? 'opacity-70' : '' }}">{{ $item->subject }}</h4>
