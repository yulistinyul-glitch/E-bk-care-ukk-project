...
<option value="{{ $t->nama_kelas }}"
{{ request('tingkatan_pilih') == $t->nama_kelas ? 'selected' : '' }}>
{{ $t->nama_kelas }}
</option>
@endforeach
...
{{ !request('tingkatan_pilih') ? 'disabled' : '' }}>

<option value="">Pilih...</option>

@foreach($jurusan_list as $j)
<option value="{{ $j->jurusan }}"
{{ request('jurusan_pilih') == $j->jurusan ? 'selected' : '' }}>
{{ $j->jurusan }}
</option>
@endforeach
...
class="form-select"
onchange="this.form.submit()"
{{ !request('jurusan_pilih') ? 'disabled' : '' }}>
...
<option value="{{ $kat->kategori_pelanggaran }}"
{{ request('kategori_pilih') == $kat->kategori_pelanggaran ? 'selected' : '' }}>
...
{{ request('id_pelanggaran') == $j->id_pelanggaran ? 'selected' : '' }}>
{{ $j->jenis_kegiatan }}
</option>
@endforeach
...
class="form-control"
readonly
value="{{ $detail ? strtoupper($detail->tingkatan) : '-' }}">
</div>

<div class="col-md-6 mb-3">
<label class="form-label">Poin</label>
<input type="text"
class="form-control text-danger-custom"
readonly
value="{{ $detail ? $detail->poin_pelanggaran : '0' }}">
</div>
...
<div class="file-hint">
Format: JPG, PNG, atau MP4 (Maks. 10MB)
</div>
...
class="form-control"
rows="2"
placeholder="Tulis rincian kejadian jika perlu...">
</textarea>
...
<a href="{{ route('gurubk.riwayatpelanggaran.index') }}" class="btn-kembali">
Batal
</a>

<button type="submit"
class="btn-simpan"
{{ !request('id_siswa') || !request('id_pelanggaran') ? 'disabled' : '' }}>
