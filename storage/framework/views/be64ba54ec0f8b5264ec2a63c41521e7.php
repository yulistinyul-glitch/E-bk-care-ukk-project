<div class="fixed bottom-0 left-0 justify-center w-full z-50 bg-[#B1D0E0] rounded-t-4xl lg:hidden">
    <nav class="w-full flex justify-around items-center p-6">
        ...
        <a href="<?php echo e(route('siswa.history')); ?>" class="group flex items-center justify-center w-14 h-14 rounded-full transition-all duration-500 ease-in-out hover:w-40 hover:bg-[#1A374D] hover:text-white overflow-hidden">
        ...
        <a href="<?php echo e(route('siswa.home')); ?>" class="group flex items-center justify-center w-14 h-14 rounded-full transition-all duration-500 ease-in-out hover:w-40 hover:bg-[#1A374D] hover:text-white  overflow-hidden
                                            <?php echo e(request()->routeIs('home') ? 'bg-[#1A374D] text-white w-8' : ''); ?>">
          <div class="flex justify-center shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" 
                   class="fill-current text-[#1A374C] group-hover:text-white transition-colors duration-500
                          <?php echo e(request()->routeIs('home') ? 'text-white' : 'text-[#1A374C] group-hover:text-white'); ?>">
        ...
        <a href="<?php echo e(route('siswa.profile')); ?>" class="group flex items-center justify-center w-14 h-14 rounded-full transition-all duration-500 ease-in-out hover:w-40 hover:bg-[#1A374C] hover:text-white overflow-hidden">
          <div class="flex justify center shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" class="fill-current text-[#1A374C] group-hover:text-white transition-colors duration-500">
...
<div class="hidden lg:flex fixed top-10 left-5 h-auto z-50">
  <aside id="sidebar-container" 
         class="py-6 px-4 rounded-full flex flex-col items-center shadow-2xl border-2 border-[#B1D0E0] transition-all duration-700 ease-in-out max-h-[85px] gap-0 relative"
         style="background: linear-gradient(to bottom, #FFFFFF, #669FC9, #1A374D);">
    
    <button id="logo-trigger" 
            class="bg-white rounded-full border-2 border-slate-800 p-1 w-12 h-12 shadow-inner flex items-center justify-center overflow-hidden transition-transform duration-500 z-10 focus:outline-none">
        <span class="text-[10px] font-bold text-slate-800 pointer-events-none">LOGO</span>
    </button>

    <nav id="nav-content" 
         class="flex flex-col gap-6 pt-6 opacity-0 transition-all duration-500 pointer-events-none overflow-hidden">
        
        <div class="group relative flex items-center justify-center">
            <a href="<?php echo e(route('siswa.home')); ?>" class="p-3 rounded-full text-white hover:bg-white/20 transition-all">
            ...
            <span class="absolute left-16 scale-0 group-hover:scale-100 transition-all origin-left bg-white text-[#1A374D] text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap z-[60]">Beranda</span>
        </div>

        <div class="group relative flex items-center justify-center">
            <a href="<?php echo e(route('siswa.profile')); ?>" class="p-3 rounded-full text-white hover:bg-white/20 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
            ...
            <span class="absolute left-16 scale-0 group-hover:scale-100 transition-all origin-left bg-white text-[#1A374D] text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap z-[60]">Profil Saya</span>
        </div>

        <div class="group relative flex items-center justify-center">
            <a href="<?php echo e(route('siswa.history')); ?>" class="p-3 rounded-full text-white hover:bg-white/20 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M13.26 3C8.17 2.86 4 6.95 4 12H2.21c-.45 0-.67.54-.35.85l2.79 2.8c.2.2.51.2.71 0l2.79-2.8a.5.5 0 0 0-.36-.85H6c0-3.9 3.18-7.05 7.1-7c3.72.05 6.85 3.18 6.9 6.9c.05 3.91-3.1 7.1-7 7.1c-1.61 0-3.1-.55-4.28-1.48a.994.994 0 0 0-1.32.08c-.42.42-.39 1.13.08 1.49A8.86 8.86 0 0 0 13 21c5.05 0 9.14-4.17 9-9.26" />
                </svg>
            </a>
      ...
      <div class="group relative flex items-center justify-center border-t border-white/20 pt-4">
        <form id="logout-form-sidebar" action="<?php echo e(route('logout')); ?>" method="POST" class="hidden">
            <?php echo csrf_field(); ?>
        </form>
...
<script>
  ...
  logoTrigger.addEventListener('click', () => {
    // Mengecek apakah sidebar sedang terbuka (max-height sudah besar atau belum)
    const isOpen = sidebarContainer.classList.contains('max-h-[600px]');

    if (!isOpen) {
      // PROSES MEMBUKA
      sidebarContainer.classList.remove('max-h-[85px]', 'gap-0');
      sidebarContainer.classList.add('max-h-[600px]', 'gap-6');
      
      logoTrigger.classList.add('rotate-180', 'scale-110');
      
      navContent.classList.remove('opacity-0', 'pointer-events-none');
      navContent.classList.add('opacity-100');
    } else {
      // PROSES MENUTUP
      sidebarContainer.classList.remove('max-h-[600px]', 'gap-6');
      sidebarContainer.classList.add('max-h-[85px]', 'gap-0');
      
      logoTrigger.classList.remove('rotate-180', 'scale-110');
      
      navContent.classList.remove('opacity-100');
      navContent.classList.add('opacity-0', 'pointer-events-none');
    }
  });
</script><?php /**PATH D:\e-bk-care-venusvault\resources\views/partials/navbar.blade.php ENDPATH**/ ?>