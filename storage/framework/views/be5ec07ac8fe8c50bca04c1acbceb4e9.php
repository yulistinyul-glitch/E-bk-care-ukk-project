...
<body class="font-['Poppins'] min-h-screen flex items-center justify-center p-4"
 ...
 <div class="w-full max-w-md md:max-w-4xl flex flex-col md:flex-row md:gap-16 items-center p-4 md:p-8 md:border-2 md:border-white/40 md:rounded-[40px] md:backdrop-blur-sm ">
     ...
     <div class="bg-white w-full max-w-95 shadow-2xl rounded-4xl p-8 md:p-10">
        ...
        <div id="form-siswa" class="space-y-4 transition-all duration-300">
            ...
            <?php if($errors->any()): ?>
                <div class="bg-red-100 text-red-600 p-3 rounded-xl text-xs mb-2"><?php echo e($errors->first()); ?></div>
            <?php endif; ?>
            <?php if(session('success')): ?>
                <div class="bg-green-100 text-green-600 p-3 rounded-xl text-xs mb-2"><?php echo e(session('success')); ?></div>
            <?php endif; ?>

            <form action="<?php echo e(route('siswa.login.submit')); ?>" method="POST" autocomplete="off">
                <?php echo csrf_field(); ?>
                ...
                <div class="mb-6 text-right">
                    <a href="<?php echo e(route('siswa.forgot-password')); ?>" class="text-xs text-[#1A374D] font-bold hover:underline">Lupa Password?</a>
                </div>
        ...
        <div id="form-guru" class="hidden space-y-4 transition-all duration-300">
            <form action="<?php echo e(route('gurubk.login.submit')); ?>" method="POST" autocomplete="off">
                <?php echo csrf_field(); ?>
 ...
 <style>
    .tab-active { color: white !important; transition: color 0.3s ease; }
    .tab-inactive { color: #6b7280 !important; transition: color 0.3s ease; }
 </style>
 ...
 <script>
    ...
    window.onload = function() {
        ...
        <?php if($errors->any() && old('role') == 'guru'): ?>
            switchRole('guru');
        <?php endif; ?>
    }
 </script>
