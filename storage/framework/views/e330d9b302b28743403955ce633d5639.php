...
<div class="container-fluid py-5 px-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5">
        ...
        <div class="badge bg-white text-dark border shadow-sm px-4 py-2" style="border-radius: 12px;">
            <i class="bi bi-calendar3 me-2 text-primary"></i><?php echo e(date('d F Y')); ?>
        ...
        <form action="<?php echo e(route('gurubk.e_surat.store')); ?>" method="POST">
            ...
            <div class="row g-4">
                <div class="col-md-3">
                    ...
                    <select name="id_siswa" id="siswa_select" class="form-select form-control-custom" onchange="isiOtomatis()" required>
                        ...
                        <?php $__currentLoopData = $siswa; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($s->id_siswa); ?>" 
                                    data-nipd="<?php echo e($s->NIPD); ?>" 
                                    data-kelas="<?php echo e($s->kelas->nama_lengkap ?? '-'); ?>"
                                    data-walikelas="<?php echo e($s->kelas->walikelas->nama_guru ?? 'Belum Set'); ?>">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="nomor_surat_resmi" class="form-control form-control-custom border-danger fw-bold text-center" value="<?php echo e($nomorOtomatis); ?>" required>
                </div>
                <div class="col-md-9">
            ...
            <input type="hidden" name="id_template" value="<?php echo e($template->first()->id_template ?? ''); ?>">
            <input type="hidden" name="id_gurubk" value="<?php echo e($gurubk->first()->id_gurubk ?? ''); ?>">
            <input type="hidden" name="tanggal_terbit" value="<?php echo e(date('Y-m-d')); ?>">
        </form>
    </div>
        ...
        <div class="table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4 fw-bold text-danger"><?php echo e($item->nomor_surat_resmi); ?></td>
                        <td>
                            <div class="fw-bold"><?php echo e($item->siswa->nama_siswa ?? 'N/A'); ?></div>
                            <small class="text-muted"><?php echo e($item->siswa->kelas->nama_lengkap ?? '-'); ?></small>
                        </td>
                        <td class="text-center">
                            <?php if($item->status == 'sent'): ?>
                                <span class="status-badge status-sent"><i class="bi bi-send-check-fill"></i> TERKIRIM</span>
                            <?php elseif($item->status == 'cetak_pdf'): ?>
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <a href="<?php echo e(route('gurubk.e_surat.print_pdf', $item->id_surat)); ?>" class="btn btn-outline-danger btn-action" target="_blank">
                                    <i class="bi bi-printer"></i> VIEW PDF
                                </a>

                                <?php if($item->status !== 'sent'): ?>
                                    <?php $emailWali = $item->siswa->kelas->walikelas->email ?? null; ?>
                                    <form action="<?php echo e(route('gurubk.e_surat.send_email', $item->id_surat)); ?>" method="POST" class="m-0">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="btn btn-success btn-action" 
                                            <?php echo e(!$emailWali ? 'disabled' : ''); ?>

                                            title="<?php echo e(!$emailWali ? 'Email Wali Kelas Belum Diatur' : 'Kirim Laporan ke Wali Kelas'); ?>">
            ...
            <div class="mt-4"><?php echo e($surat->links()); ?></div>
        </div>
    </div>
</div>
...
<?php echo $__env->make('gurubk.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\e-bk-care-venusvault\resources\views/gurubk/e_surat/index.blade.php ENDPATH**/ ?>