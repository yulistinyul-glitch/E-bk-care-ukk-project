...
<div class="relative w-full md:w-1/2 h-[350px] md:h-auto bg-gradient-to-br from-[#0f2027] via-[#3f6d7c] to-[#3e748b] text-white">
    ...
    <div class="w-full md:w-1/2 p-8 text-blue-950">
        ...
        <form id="loginForm" action="{{ route('admin.login.submit')}}" method="POST" autocomplete="off">
            @csrf

            @if ($errors->has('username'))
                <input type="hidden" id="loginError" value="{{ $errors->first('username') }}">
            @endif
...
<body class="font-['Poppins'] min-h-screen flex items-center justify-center p-4"
    ...
    <div class="w-full max-w-3xl flex flex-col md:flex-row rounded-4xl overflow-hidden shadow-2xl bg-white">

        <div class="relative w-full md:w-1/2 h-[350px] md:h-auto">
            <img src="{{ asset('img/admin.jpg') }}" class="w-full h-full object-cover">
        ...
        <div class="w-full md:w-1/2 p-8 text-blue-950">
            ...
            <form id="loginForm" action="{{ route('admin.login.submit')}}" method="POST" autocomplete="off">
                ...
                <button type="submit" id="loginBtn"
                    class="w-full flex items-center justify-center gap-2 bg-[#1A374D] text-white text-sm font-semibold py-3 rounded-2xl hover:bg-[#2c4e66] transition-all duration-300 active:scale-95 shadow-lg">

                    <span id="btnText">Masuk ke Dashboard</span>

                    <svg id="loadingIcon" class="hidden w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle cx="12" cy="12" r="10" stroke="white" stroke-width="4"></circle>
                        <path fill="white" d="M4 12a8 8 0 018-8v8H4z"></path>
                    </svg>
                </button>
...
<div id="alertModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    ...
    <div id="modalBox"
        class="bg-white rounded-3xl w-[300px] h-[240px] flex flex-col items-center justify-center text-center shadow-xl p-6">

        <!-- ICON -->
        <div id="modalIcon" class="text-6xl mb-4 animate-bounce-slow">
            ⚠️
        </div>

        <!-- TITLE -->
        <h3 id="modalTitle" class="font-bold text-xl text-[#1A374D] mb-2">
            Oops!
        </h3>

        <!-- TEXT -->
        <p id="modalText" class="text-sm text-gray-500 leading-relaxed px-2">
            Pesan error
        </p>
